...
export class WalrusService {
  ...
  static async uploadFromUrl(url: string, userAddress?: string): Promise<WalrusBlob> {
    try {
      ...
      // Extract blob ID from response
      const blobId =
        walrusResponse.data.newlyCreated?.blobObject.blobId ||
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (error.code === 'ECONNABORTED' || error.message.includes('timeout')) {
          throw new Error(
            'Timeout: The URL took too long to respond. Please try a different image or upload directly.'
          );
        }

        if (error.response?.status === 403) {
          throw new Error(
            'Access denied: The URL requires authentication or blocks automated access.'
          );
        }

        if (error.response?.status === 404) {
          throw new Error(
            'Not found: The URL does not exist or the image has been removed.'
          );
        }

        if (error.response && error.response.status >= 500) {
          throw new Error(
            "Server error: The URL's server is experiencing issues. Please try again later."
          );
        }
      ...
      throw new Error(
        `Failed to fetch and upload URL: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
   ...
   */
  static async checkBlobAvailability(
    blobId: string,
    maxRetries: number = 3,
    delayMs: number = 2000
  ): Promise<boolean> {
  ...
  static async uploadMetadata(metadata: any, userAddress?: string): Promise<WalrusBlob> {
    ...
    try {
      ...
      const response = await axios.put<WalrusUploadResponse>(uploadUrl, binaryData, {
        headers: {
          'Content-Type': 'application/octet-stream',
        },
        timeout: 60000,
   ...
   */
  static validateUrlForUpload(url: string): {
    isValid: boolean;
    message: string;
    suggestions: string[];
  } {
