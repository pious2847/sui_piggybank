...
export const getWalrusUrl = (blobId: string): string => WalrusService.getBlobUrl(blobId);

export const fetchFromWalrus = async (blobId: string, options?: { skipCache?: boolean }): Promise<string> => {
  try {
    const url = WalrusService.getBlobUrl(blobId);
    const response = await axios.get(url, {
      timeout: 10000,
    ...
    if (axios.isAxiosError(error)) {
      throw new WalrusError(`Failed to fetch from Walrus: ${error.message}`, error.code);
    }
    throw new WalrusError('Failed to fetch from Walrus');
  }
};

export const fetchJSONFromWalrus = async <T = any>(blobId: string, options?: { skipCache?: boolean }): Promise<T> => {
  try {
    const url = WalrusService.getBlobUrl(blobId);
    ...
    if (axios.isAxiosError(error)) {
      throw new WalrusError(`Failed to fetch JSON from Walrus: ${error.message}`, error.code);
    }
    throw new WalrusError('Failed to fetch JSON from Walrus');
  }
};

export const fetchBlobFromWalrus = async (blobId: string, options?: { skipCache?: boolean }): Promise<Blob> => {
  try {
    const url = WalrusService.getBlobUrl(blobId);
    const response = await axios.get(url, {
      responseType: 'blob',
    ...
    if (axios.isAxiosError(error)) {
      throw new WalrusError(`Failed to fetch blob from Walrus: ${error.message}`, error.code);
    }
    throw new WalrusError('Failed to fetch blob from Walrus');
  }
};
...
export const getWalrusConfig = (): WalrusConfig => ({
...
export const checkWalrusHealth = async (): Promise<boolean> => {
  return WalrusService.checkServiceHealth();
}; 